...
public class DownLoadExcel {
		...
		try {
			...
			if ( null != datas && datas.size() > 0 ) {
				...
				for ( int i = 0 ; i< datas.size(); i++) {
					List<EntityCellValues> entityCells =  downloadExcutor.getCellValues(datas.get(i));
					if ( null != entityCells ) {
						for (int j = 0 ; j < entityCells.size(); j++) {
							rowIndex ++;
							Row row = sheet.createRow(rowIndex);
							EntityCellValues ecv = entityCells.get(j);
							List<String> cells = ecv.getCellValues();
							if ( null != cells ) {
	...
	public static void download(List datas,List<String> titles,DownLoadExcutor downloadExcutor,HttpServletResponse response) {
        try {
            Workbook workbook = getWorkbook(datas,titles,downloadExcutor);
            String filename = com.simple.common.excel.DateUtil.date2StringWhitNoSpiltSeconds(new Date())+"."+ExcelUtil.OFFICE_EXCEL_2010_POSTFIX;
...
//	}
	...
	public static void main(String[] args) {
		...
		SheetCell titleCell = new SheetCell("调查问卷",1,3);
